...
def get_last_reqs():
    ...
    """
    cur = g.db.execute('select reqdata from data order by created_on desc limit 25')
...
def upload_data():
    ...
    if request.method == 'POST':
		...
		foodBoolean=request.data['food']
		clothBoolean=request.data['cloth']
		medicineBoolean=request.data['medicine']
		latitude=request.data['latitude']
		longitude=request.data['longitude']
		special= request.data['special']
		f = "Name:"+name+ " Phone Number: "+ phoneNumber + " No of people: "+ numPeople +" Food: "+foodBoolean+" Cloth: "+clothBoolean+" Medicine: "+medicineBoolean + " Latitude: " + latitude + " Longitude: " + longitude + " Special: "+special 
		print f	
		add_data(f)
		return "ok"
